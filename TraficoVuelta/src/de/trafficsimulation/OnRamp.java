...
public class OnRamp extends MicroStreet{
    ...
    public OnRamp(MicroStreet mainroad, double totalLength, 
                  double rampLength, double mergingPosAtMainroad, double p_factorRamp, double deltaBRamp){
    
	super(totalLength, 0., p_factorRamp, deltaBRamp, 0, 2);
	// -> roadlength=totalLength, choice_Szen=2,
        // en SimCanvas totalLength=total_rmplen_m =221, linea 495
    ...
    private void mergeToMainRoad(LaneChange laneChangeModel,
       ...
       final double bmax=4.;  // maximum save braking deceleration
        ...
        final double offsetMain =  mergingPos - (roadLength-0.5*rampLength);//851,52-(221-50)=680
	int imax=street.size()-1;
        //System.out.println("in mergeToMainRoad: street= "+imax);
	int nvehmain=mainroad.street.size();
        //System.out.println("in mergeToMainRoad: mainroad.street= "+nvehmain);
	...
	if(imax>=1){  // at least one real vehicle on the ramp lane
         for (int i=1; i<=imax; i++){ 
          ...
          //System.out.println("in mergeToMainRoad: mi posicion"+x);
          //System.out.println("in mergeToMainRoad: roadLength "+roadLength+" rampLength"+rampLength);
          //double dif = roadLength-rampLength;
          //System.out.println("in mergeToMainRoad: roadLength - rampLength= "+dif);
    ...
    private void setNeighboursOnMainRoad(int i, double offsetMain){
	 ...
	 if(debug){
       ...
       frontVehMain = (i_frontmain<0) // only back vehicle(s)
          ? new Car(farDistance, 0, 0, 1,
            idmCar, inconsiderate, 5, colorCarFM, 0)
          : new Car( (Car)(mainroad.street.elementAt(i_frontmain)));


       backVehMain = ((nvehmain<1)||(i_backmain >= nvehmain)) 
	      ? new Car(-farDistance, 0, 0,1,  // only front veh(s) or none
            idmCar, inconsiderate, 5, colorCarFM, 0)
	...
	//System.out.println("in OnRamp.inflow: qIn="+qIn);
       ...
       if (nin>1.0){  // new vehicle imposed by the inflow BC
         ...
         if (!(red=(space<spaceMin))){
           ...
           Color      colorNew  = (rand<perTr) ? colorTruck : colorCarFM;

	   street.insertElementAt
		 ((new Car(0.0, vNew, lane,1, modelNew, inconsiderate, 
